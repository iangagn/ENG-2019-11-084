...
for iteration = 1 : max_iter
    ...
    % Random walk around global best
    random_walk_indexes = find(rand(nb_bats, 1) > r);
    ...
    if min(fitnesses_new) < global_best_fitness && rand() < A(best_index)
        ...
        nb_not_too_loud_indexes = numel(not_too_loud_indexes);
        A(not_too_loud_indexes) = a * A(not_too_loud_indexes);
        r(not_too_loud_indexes) = repmat(r0 *(1-exp(-g*iteration)), nb_not_too_loud_indexes, 1);
